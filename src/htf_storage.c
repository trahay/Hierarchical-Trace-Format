...
static void _htf_store_string(struct htf_string *l, int string_index);
static void _htf_store_regions(struct htf_trace *t);
...
static void _htf_read_string(struct htf_string *l, int string_index);
static void _htf_read_regions(struct htf_trace* t);
...
static FILE* _htf_get_string_file(int string_index, char* mode) {
  char filename[1024];
  snprintf(filename, 1024, "%s/string_%d", base_dirname, string_index);
  return _htf_file_open(filename, mode);
}

static void _htf_store_string(struct htf_string *s, int string_index) {
  FILE* file = _htf_get_string_file(string_index, "w");
...
static void _htf_read_string(struct htf_string *s, int string_index) {
  FILE* file = _htf_get_string_file(string_index, "r");
...
static FILE* _htf_get_regions_file(char* mode) {
  char filename[1024];
  snprintf(filename, 1024, "%s/regions.dat", base_dirname);
  return _htf_file_open(filename, mode);
}
...
static void _htf_store_regions(struct htf_trace* t) {
  ...
  FILE* file = _htf_get_regions_file("w");
  htf_log(htf_dbg_lvl_debug, "\tStore %d Regions\n", t->nb_regions);

  fwrite(t->regions, sizeof(struct htf_region), t->nb_regions, file);
  fclose(file);
}
...
static void _htf_read_regions(struct htf_trace *t) {
  ...
  FILE* file = _htf_get_regions_file("r");
  
  printf( "\tLoad %d regions\n", t->nb_regions);

  t->regions = malloc(sizeof(struct htf_region)* t->nb_regions);
  fread(t->regions, sizeof(struct htf_region), t->nb_regions, file);
  fclose(file);
...
void htf_storage_finalize(struct htf_trace*trace) {
  ...
  fwrite(&trace->nb_strings, sizeof(int), 1, main_file);
  fwrite(&trace->nb_regions, sizeof(int), 1, main_file);
  fclose(main_file);
  ...
  for(int i = 0; i<trace->nb_strings; i++) {
    _htf_store_string(&trace->strings[i], i);
  }

  _htf_store_regions(trace);
}
...
void htf_read_trace(struct htf_trace* trace, char* main_filename) {
  ...
  fread(&trace->nb_strings, sizeof(int), 1, main_file);
  fread(&trace->nb_regions, sizeof(int), 1, main_file);
  fclose(main_file);
  ...
  trace->strings = malloc(sizeof(struct htf_string) * trace->nb_strings);
  for(int i = 0; i<trace->nb_strings; i++) {
    _htf_read_string(&trace->strings[i], i);
  }

  _htf_read_regions(trace);
}
