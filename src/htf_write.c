 ...
 */
static inline htf_sequence_id_t _htf_get_sequence_id(struct htf_thread* thread_trace, struct htf_sequence* seq) {
...
static inline htf_sequence_id_t _htf_get_sequence_id_from_array(struct htf_thread* thread_trace,
  ...
  htf_log(htf_dbg_lvl_debug, "\tSequence not found. Adding it with id=S%x\n", index);
...
static inline struct htf_loop* _htf_create_loop_id(struct htf_thread_writer* thread_writer,
  ...
  DOFOR(i, thread_writer->thread_trace.nb_loops) {
    if (thread_writer->thread_trace.loops[i].token.id == sid.id) {
      index = i;
      htf_log(htf_dbg_lvl_debug, "\tLoop already exists: id=L%x containing S%x\n", index, sid.id);
      break;
    }
  }
  if (index == -1) {
    index = thread_writer->thread_trace.nb_loops++;
    htf_log(htf_dbg_lvl_debug, "\tLoop not found. Adding it with id=L%x containing S%x\n", index, sid.id);
  }
...
void htf_store_timestamp(struct htf_thread_writer* thread_writer,
  ...
  if (occurrence_index >= es->nb_allocated_events) {
    ...
    htf_assert(occurrence_index < es->nb_allocated_events);
  }

  es->durations[occurrence_index] = ts;
  htf_delta_timestamp(&es->durations[occurrence_index]);
}
...
static inline htf_timestamp_t _htf_get_sequence_duration(struct htf_thread* thread, htf_token_t* array, int size) {
  int* event_index = calloc(thread->nb_events, sizeof(int));
  ...
  for (int i = size - 1; i >= 0; i--) {
    ...
    case HTF_TYPE_EVENT: {
      ...
      duration += es.durations[es.nb_occurrences - event_index[t.id]];
      break;
    }
...
static void _htf_find_loop(struct htf_thread_writer* thread_writer) {
  ...
  for (int loop_len = 1; loop_len < MAX_LOOP_LENGTH; loop_len++) {
    ...
    if (s1_start > 0) {
      ...
      if (HTF_TOKEN_TYPE(cur_seq->token[loop_start]) == HTF_TYPE_LOOP) {
        ...
        struct htf_sequence* seq =
          htf_get_sequence(&thread_writer->thread_trace, HTF_TOKEN_TO_SEQUENCE_ID(loop->token));
        htf_assert(seq);
        ...
        if (_htf_arrays_equal(&cur_seq->token[s1_start], loop_len, seq->token, seq->size)) {
          ...
          htf_log(htf_dbg_lvl_debug, "Last tokens were a sequence from L%x aka S%x\n", loop->id.id, loop->token.id);
          _htf_loop_add_iteration(loop);
          htf_timestamp_t ts =
    ...
    if (s2_start >= 0) {
      ...
      is_loop = _htf_arrays_equal(&cur_seq->token[s1_start], loop_len, &cur_seq->token[s2_start], loop_len);

      if (is_loop) {
        if (htf_debug_level >= htf_dbg_lvl_debug) {
          printf("Found a loop of len %d:\n", loop_len);
          htf_print_token_array(&thread_writer->thread_trace, cur_seq->token, s1_start, s1_start + loop_len);
          htf_print_token_array(&thread_writer->thread_trace, cur_seq->token, s2_start, s2_start + loop_len);
          printf("\n");
        }
...
int htf_store_event(struct htf_thread_writer* thread_writer,
  ...
  htf_store_timestamp(thread_writer, es, ts, occurrence_index);
  if(attribute_list)
...
struct htf_location_group* htf_get_location_group(struct htf_archive* archive, htf_location_group_id_t location_group) {
...
struct htf_location* htf_get_location(struct htf_archive* archive, htf_thread_id_t id) {
...
static void _init_thread(struct htf_archive* archive, struct htf_thread* t, htf_thread_id_t thread_id) {
  t->archive = archive;
  t->id = thread_id;
...
void htf_write_global_archive_open(struct htf_archive* archive, const char* dir_name, const char* trace_name) {
