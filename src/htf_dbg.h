...
#define htf_warn(format, ...)                                                                                \
  do {                                                                                                       \
    _htf_log(stderr, htf_dbg_lvl_normal, "HTF warning in %s (%s:%d): " format, __func__, __FILE__, __LINE__, \
             ##__VA_ARGS__);                                                                                 \
  } while (0)

#define htf_error(format, ...)                                                                            \
  do {                                                                                                    \
    _htf_log(stderr, htf_dbg_lvl_error, "HTF error in %s (%s:%d): " format, __func__, __FILE__, __LINE__, \
             ##__VA_ARGS__);                                                                              \
    htf_abort();                                                                                          \
  } while (0)
